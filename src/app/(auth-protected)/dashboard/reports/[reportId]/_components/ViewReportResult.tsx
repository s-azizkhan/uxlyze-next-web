    ...
    <div className="min-h-screen">
      ...
      >
              ...
              reportData.reportConfig.includeAIAnalysis && (
                ...
                >
                  <div className="w-24 h-24 sm:w-32 sm:h-32 mb-2 sm:mb-4">
                    <CircularProgressbar
                      value={
                        reportData.resultData.result.AiAnalysis.total_score * 10
                      }
                      text={`${reportData.resultData.result.AiAnalysis.total_score.toFixed(
                        1
                      )}`}
                      styles={buildStyles({
                        textSize: "24px",
                  ...
                  <span className="text-xs sm:text-sm mt-1 sm:mt-2 px-2 sm:px-3 py-1 bg-white/20 rounded-full">
                    {reportData.resultData.result.AiAnalysis.website_category}
                  </span>
                </motion.div>
              )}
          </div>
        ...
        {reportData.status !== "completed" ? (
          ...
          <CompletedStatusReport report={report} />
        )}
      </motion.div>
    </div>
  );
...
function CompletedStatusReport({ report }: any) {
  return (
    <>
      <motion.div
        initial={{ opacity: 0, y: 20 }}
